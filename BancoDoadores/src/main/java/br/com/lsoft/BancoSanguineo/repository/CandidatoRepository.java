...
public interface CandidatoRepository extends CrudRepository<Candidato, String>{
	...
	@Query(nativeQuery = true, value = "select sexo, obesos, total, obesos/total * 100 as representacao from (select c.sexo, sum(case when (c.peso / c.altura / c.altura) > 30 then 1 else 0 end) as obesos, count(sexo) as total from candidato c group by sexo) as resultset")
	public List<Object> findObesosPorSexo();

	@Query(nativeQuery = true, value = "select tipo_sanguineo, avg(idade) from (select tipo_sanguineo, floor(datediff(curdate(), c.data_nasc) / 365) as idade from candidato c) as resultset group by tipo_sanguineo")
	public List<Object> findMediaPorTipoSanguineo();

	@Query(nativeQuery = true, 
			value = "select distinct\n"
			+		"	c.tipo_sanguineo as 'tipo_sanguineo_destino', cdp.*\n"
			+ 		"	from Candidato c\n"
	...
	public List<Object> findDoadoresDisponiveis();
